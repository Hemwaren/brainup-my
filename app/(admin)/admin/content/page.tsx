...
// ============ CONSTANTS ============
const PILLARS = ["KNOW_YOURSELF", "CHOOSE_YOURSELF", "GIVE_YOURSELF"];
const PILLAR_LABELS: Record<string, string> = {
  KNOW_YOURSELF: "Know Yourself",
  CHOOSE_YOURSELF: "Choose Yourself",
  GIVE_YOURSELF: "Give Yourself",
};

const RESOURCE_TYPES = ["ARTICLE", "VIDEO", "WORKSHEET", "QUIZ"];
const RESOURCE_STATUSES = ["PUBLISHED", "DRAFT", "ARCHIVED"];
...
function EIResourcesTab() {
  ...
  async function handleSubmit() {
    if (!form.title.trim() || !form.description.trim()) {
      setMsg({ text: "Title and description are required.", type: "error" });
      return;
    }
    setSaving(true);
    const payload = {
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-4 text-sm font-extrabold text-slate-900">{editingId ? "Edit Resource" : "Add New Resource"}</div>
          <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <label className="text-xs font-bold text-slate-500 block mb-1">Title *</label>
                <input value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} placeholder="Resource title..." className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-cyan-300" />
              </div>
              <div>
                <label className="text-xs font-bold text-slate-500 block mb-1">Type</label>
                <select value={form.type} onChange={e => setForm(p => ({ ...p, type: e.target.value }))} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-cyan-300">
            ...
            <div>
              ...
              <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} rows={2} placeholder="Brief description..." className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-cyan-300 resize-none" />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-500 block mb-1">Content (optional)</label>
              <textarea value={form.content} onChange={e => setForm(p => ({ ...p, content: e.target.value }))} rows={4} placeholder="Full article content..." className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-cyan-300 resize-none" />
            </div>
            <div className="grid gap-4 md:grid-cols-3">
              <div>
                <label className="text-xs font-bold text-slate-500 block mb-1">EI Pillar</label>
                <select value={form.pillar} onChange={e => setForm(p => ({ ...p, pillar: e.target.value }))} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-cyan-300">
              ...
              <div>
                ...
                <select value={form.status} onChange={e => setForm(p => ({ ...p, status: e.target.value }))} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-cyan-300">
              ...
              <div>
                ...
                <input type="date" value={form.publish_date} onChange={e => setForm(p => ({ ...p, publish_date: e.target.value }))} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-cyan-300" />
              </div>
            </div>
          </div>
          {msg && <p className={`mt-3 text-sm font-semibold ${msg.type === "error" ? "text-rose-600" : "text-emerald-600"}`}>{msg.text}</p>}
          <div className="mt-4 flex gap-3">
            <button type="button" onClick={handleSubmit} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-teal-500 via-cyan-500 to-sky-500 px-5 py-2.5 text-sm font-extrabold text-white hover:opacity-95 disabled:opacity-50 transition">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={s.label} className="glow-card rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
              ...
              <div key={r.id} className="flex items-start gap-4 px-5 py-4 hover:bg-slate-50 transition">
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    ...
                    <span className="inline-flex items-center rounded-full border border-slate-200 px-2 py-0.5 text-[10px] font-extrabold text-slate-600">{r.type}</span>
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-extrabold ${r.status === "PUBLISHED" ? "bg-emerald-50 text-emerald-700" : r.status === "DRAFT" ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-500"}`}>{r.status}</span>
                    <span className="inline-flex items-center rounded-full bg-cyan-50 border border-cyan-200 px-2 py-0.5 text-[10px] font-extrabold text-cyan-700"><Tag size={8} className="mr-1" />{PILLAR_LABELS[r.pillar]}</span>
                  </div>
                ...
                <div className="flex gap-2 shrink-0">
                  ...
                  <button type="button" onClick={() => handleDelete(r.id)} disabled={deletingId === r.id} className="grid h-8 w-8 place-items-center rounded-xl border border-rose-200 bg-rose-50 text-rose-500 hover:bg-rose-100 disabled:opacity-50 transition">
...
function JournalQuotesTab() {
  ...
  async function handleSubmit() {
    if (!form.quote.trim()) {
      setMsg({ text: "Quote text is required.", type: "error" });
      return;
    }
    setSaving(true);
    const payload = {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={openCreate} className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-teal-500 via-cyan-500 to-sky-500 px-4 py-2 text-sm font-extrabold text-white shadow-sm hover:opacity-95 transition">
      ...
      {showForm && (
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="space-y-4">
            <div>
              <label className="text-xs font-bold text-slate-500 block mb-1">Quote *</label>
              <textarea value={form.quote} onChange={e => setForm(p => ({ ...p, quote: e.target.value }))} rows={3} placeholder="Enter the quote text..." className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-cyan-300 resize-none" />
            </div>
            <div className="grid gap-4 md:grid-cols-3">
              <div>
                <label className="text-xs font-bold text-slate-500 block mb-1">Author</label>
                <input value={form.author} onChange={e => setForm(p => ({ ...p, author: e.target.value }))} placeholder="e.g. Brené Brown" className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-cyan-300" />
              </div>
              <div>
                <label className="text-xs font-bold text-slate-500 block mb-1">Mood Category</label>
                <select value={form.mood_category} onChange={e => setForm(p => ({ ...p, mood_category: e.target.value }))} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-cyan-300">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.is_active} onChange={e => setForm(p => ({ ...p, is_active: e.target.checked }))} className="h-4 w-4 rounded border-slate-300" />
          ...
          <div className="divide-y divide-slate-100">
            {quotes.map(q => (
              <div key={q.id} className="flex items-start gap-4 px-5 py-4 hover:bg-slate-50 transition">
                ...
                <div className="flex gap-2 shrink-0">
                  <button type="button" onClick={() => toggleActive(q)} className={`inline-flex items-center gap-1 rounded-xl border px-2.5 py-1.5 text-xs font-extrabold transition ${q.is_active ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-slate-200 bg-slate-100 text-slate-500"}`}>
...
function GamificationTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      {msg && (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-700">{msg}</div>
      )}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          {activeMissions.map(m => (
            <MissionRow key={m.id} mission={m} onToggle={toggleMission} onXPChange={updateXP} saving={saving === m.id} />
          ))}
        </div>
      </section>
  ...
  return (
    <div className={`flex items-center gap-4 px-5 py-3 hover:bg-slate-50 transition ${!m.is_active ? "opacity-60" : ""}`}>
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-1 inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-extrabold text-slate-600">
          {m.activity_key}
        </div>
      </div>
      ...
      <button
        ...
        disabled={saving}
        className={[
          "inline-flex items-center gap-1 rounded-xl border px-3 py-1.5 text-xs font-extrabold transition",
          m.is_active
